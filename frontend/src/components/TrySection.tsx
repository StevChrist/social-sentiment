// src/components/TrySection.tsx - FIXED DISPLAY VERSION
"use client";

import React, { useEffect, useRef, useState } from "react";
import { startScrape, getScrapeStatus, JobStatus, AnalyzeOut } from "@/lib/api";
...
export default function TrySection(): React.ReactElement {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      // ✅ Direct API call - get result immediately
      const response = await startScrape(url, percentage);
...
Alternatively, try clearing your browser cache.`);
  ...
  return (
    ...
    >
      ...
      {/* ✅ RESULTS SECTION - Show when result is available */}
      {result && (
        <div style={{
          marginTop: "24px", 
          width: "100%", 
          ...
          <div style={{ marginBottom: "20px", color: "rgba(245,245,245,0.85)" }}>
            ...
            <div style={{ fontSize: "14px" }}>
              Analyzed: {result.actual_analyzed} comments ({(result.percentage_analyzed * 100).toFixed(0)}% of {result.total_comments} total)
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: "#10B981" }}>
                {result.counts.positive} ({(result.ratios.positive * 100).toFixed(1)}%)
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: "#F59E0B" }}>
                {result.counts.neutral} ({(result.ratios.neutral * 100).toFixed(1)}%)
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontWeight: 700, color: "#EF4444" }}>
                {result.counts.negative} ({(result.ratios.negative * 100).toFixed(1)}%)
              </div>
            </div>
          </div>
